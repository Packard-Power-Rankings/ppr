...
def update_record(home_team_id, visitor_team_id, week_id):
    try:
        last_home_weekly_data = hs_models.TeamWeeklyData.objects.filter(
            ...
            week_id__season_id=week_id.season_id).order_by("-week_id__num")[0]
    ...
    try:
        ...
        return
    # print(f'{last_home_weekly_data.team_id.name} vs
    # {last_visitor_weekly_data.team_id.name}')
    home_team_games_this_season = hs_models.Game.objects.filter
    (Q(week_id__season_id=week_id.season_id) & (
        Q(home_team_id=home_team_id) | Q(visitor_team_id=home_team_id)))
    visitor_team_games_this_season = hs_models.Game.objects.filter
    (Q(week_id__season_id=week_id.season_id) & (
        Q(home_team_id=visitor_team_id) | Q(visitor_team_id=visitor_team_id)))
    last_home_weekly_data.wins = last_home_weekly_data.losses = \
        last_home_weekly_data.ties = 0
    last_visitor_weekly_data.wins = last_visitor_weekly_data.losses = \
        last_visitor_weekly_data.ties = 0
...
def calculate_power_difference(
    ...
    visitor_power_change = actual_performance_visitor - \
    ...
    if modify_teams:
        name_to_team[game.home_team_id.name].actual_change = name_to_team
        [game.home_team_id.name].temp_actual_change
        name_to_team[game.visitor_team_id.name].actual_change = name_to_team
        [game.visitor_team_id.name].temp_actual_change

    if home_team_id is None:
        name_to_team[game.home_team_id.name].power = round(
            name_to_team[game.home_team_id.name].power + name_to_team
            [game.home_team_id.name].temp_actual_change, 5)
        name_to_team[game.visitor_team_id.name].power = round(
            name_to_team[game.visitor_team_id.name].power + name_to_team
            [game.visitor_team_id.name].temp_actual_change, 5)
    else:
...
def calculate_expected_performance(
    ...
    if home_team_id is None:
        ...
        expected_visitor_wl = (
            visitor_team_id.power - home_team_id.power +
            (-1.0 * home_field_advantage))
...
def modify_gameset(
    ...
    if game.original_home_score - game.original_visitor_score >= 0:
        modified_home_score = (
            10.0 * TOTAL + 9.0 * game.original_home_score - 10.0 *
            game.original_visitor_score) / 19.0
        ...
        modified_visitor_score = (
            10.0 * TOTAL + 9.0 * game.original_visitor_score - 10.0 *
            game.original_home_score) / 19.0
            ...
            for k in range(0, 6):
                ...
                for m in range(0, 6):
                    ...
                    if m == 0:
                        ...
                        continue
                    for n in range(0, 6):
                        if depth_3.recent_opponents[n] == 0:
                            continue
                        try:
                            depth_4 = d[depth_3.recent_opponents[n]]
                        except BaseException:
                            continue
                        if n == 0:
                            depth_4.power += (
...
def sort_teams_by_division(teams):
    ...
    for team in teams:
        ...
        if (team.team_id.state_id.name == "Colorado" or team.team_id.level !=
                "High School") and team.team_id.division_id is not None:
...
def standard_deviation(name_to_team, games):
    # games = hs_models.Game.objects.filter(week_id__season_id=season).exclude
    # (home_team_id__isnull=True, visitor_team_id__isnull=True)
    num_games = 0
    if games.count() == 0:
        return 0.0
    actual_change_sum = 0
    for game in games:
        if name_to_team[game.home_team_id.name].temp_actual_change != 0:
            actual_change_sum += name_to_team
            [game.home_team_id.name].temp_actual_change ** 2
            num_games += 1
        if name_to_team[game.visitor_team_id.name].temp_actual_change != 0:
            actual_change_sum += name_to_team
            [game.visitor_team_id.name].temp_actual_change ** 2
            num_games += 1
...
def get_z_score(games, name_to_team):
    ...
    num_games = 0
    for game in games:
        name_to_team[game.home_team_id.name].temp_actual_change_list = \
            name_to_team[game.home_team_id.name].actual_change_list.copy()
        name_to_team[game.visitor_team_id.name].temp_actual_change_list = \
            name_to_team[game.visitor_team_id.name].actual_change_list.copy()
    for game in games:
        if (name_to_team[game.home_team_id.name].temp_actual_change_list[0]) \
                != 0:
            actual_change_sum += name_to_team
            [game.home_team_id.name].temp_actual_change_list[0] ** 2
            actual_change_sum += name_to_team
            [game.visitor_team_id.name].temp_actual_change_list[0] ** 2
            num_games += 2
...
def p(absolute_power_difference):
    return 1.0 / (1.0 + 40.0 * math.e **
                  (-1.0 * ((absolute_power_difference / 6.0) + 2.0)))


def predict_scores(
        home_team_weekly_stats,
        visitor_team_weekly_stats,
        hfa,
        sport):
    absolute_power_difference = abs(
        home_team_weekly_stats.power - visitor_team_weekly_stats.power)
    if hfa is not None:
        if (home_team_weekly_stats.power +
                sport.home_advantage >= visitor_team_weekly_stats.power):
            absolute_power_difference += sport.home_advantage
        else:
            absolute_power_difference -= sport.home_advantage

    victory_break_even = (
        (((absolute_power_difference + 2) ** 0.45) - 1.0) ** (20.0 / 9.0))
    - 2.0
    ...
    if home_team_weekly_stats.num_games != 0 and \
            visitor_team_weekly_stats.num_games != 0:
        AVE = ((home_team_weekly_stats.total_score /
                home_team_weekly_stats.num_games) +
               (visitor_team_weekly_stats.total_score /
                visitor_team_weekly_stats.num_games))  # /2.0?
    ...
    if LOS3 < 0:
        ...
        LOS3 = 0
    if (home_team_weekly_stats.power +
        (sport.home_advantage if hfa else 0) -
            visitor_team_weekly_stats.power < 0):
        return LOS3, WIN3
