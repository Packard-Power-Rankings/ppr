...
async def list_sports(
    ...
    try:
        ...
        query: Dict = {"id": id}  # Initialize with ID as it's mandatory

        # Generate sport_doc just like in the POST route
        sport_doc: Dict = json_file_builder(sport_type, gender, level)

        # Fetch sports data using the query
        sports = await retrieve_sports(query, id)

        # Check if sports data is found
        if sports:
            team_info = sport_doc['sports'][sport_type][gender][level].get('team', [])
            ...
            return items.ResponseModel(response_data, "Sports data retrieved successfully")
        else:
